...
구현 완료·self-review clean(2026-07-17). 전체 FR-1~7·AC-1~9 충족 — 근거는 각 항목에 부기.
전체 `npm test` 853/0, `tsc` clean, 백업 셸 테스트 31/0, 도그푸드 실증 완료.
...
- [x] **FR-1 (바인딩 설정 스키마)**: 실행 등급 3종(`critical-reasoning`·`standard`·`economy`)
      ...
      — **검증**: `src/agents/binding.ts` zod 스키마(schemaVersion·runtime·updatedAt·tiers·roles)
      + `bindingsDir()` 유닛(`binding.test.ts`) + 도그푸드(격리 배포 시 `_bindings/<id>.json` 생성 관찰).
- [x] **FR-2 (전용 온보딩 진입점)**: 사용자가 명시적으로 호출하는 전용 설정 진입점을 만든다.
      ...
      — **검증**: `templates/skills/localmind-binding/SKILL.md` + catalog 편입(explicit·mutating),
      `skill-contract.test.ts` 정적 AC 검증 + 도그푸드(3타깃 재현으로 온보딩 흐름 실증).
- [x] **FR-3 (재설정)**: 온보딩 진입점을 재실행하면 기존 바인딩 값을 보여주고, 원하는 항목만
      수정해 저장할 수 있다(나머지 항목 보존).  → goal: Success metrics(재설정 가능)
      — **검증**: `mergeBinding` 부분 수정·나머지 보존 유닛(`binding.test.ts`) + 재실행 도그푸드.
- [x] **FR-4 (워크플로 스킬의 바인딩 소비 규약)**: 워크플로 스킬은 페르소나·모델이 필요한
      ...
      — **검증**: `resolveTier`/`resolveRole` 부재 사유 반환 유닛 + `binding-contract.md` D-4
      정적 검증(지침수준 — 실소비는 051, 정적+도그푸드로 확인).
- [x] **FR-5 (페르소나 fallback — 비독립 명시)**: 격리 서브에이전트 위임 능력이 없는 런타임
      ...
      — **검증**: `binding-contract.md` 페르소나 fallback(비독립) 정적 검증 — 지침수준(instruction-level)
      행동이라 실소비는 051, 이번은 정적 계약 + 도그푸드로 확인.
- [x] **FR-6 (런타임별 격리)**: 각 런타임(Claude Code·Codex·Gemini CLI 등)의 바인딩은 서로
      독립이다 — 한 런타임에서의 설정·수정이 다른 런타임의 바인딩에 영향을 주지 않고, 다른
      런타임의 바인딩을 대신 읽지 않는다.  → goal: Constraints(로컬·비동기화) / Success metrics(격리)
      — **검증**: `loadBinding` 정확 일치 유닛 + **traversal 가드**(NAME_RE) 유닛(격리 회귀 4건,
      self-review에서 발견·수정) + 백업 격리 셸 31/0.
- [x] **FR-7 (입력 검증과 우아한 실패)**: 온보딩 입력을 다음과 같이 다룬다 — ① 페르소나는
      ...
      — **검증**: `validateBinding` 유닛 — 부분설정 유효(AC-6) / 무효 persona 거부(AC-7) /
      빈 레지스트리(AC-8) / 모델 형식(AC-9) 각각 케이스로 확인.

## Acceptance Criteria

유저 시나리오:

- [x] **AC-1 (최초 설정)**: Given 바인딩이 없는 설치에서, When 사용자가 온보딩 진입점을
      실행하면, Then 등급별 모델 추천 초안(낡을 수 있음 고지 포함)과 레지스트리 기반 페르소나
      후보가 제시되고, 사용자 확정 값이 해당 런타임의 바인딩으로 저장되며, 저장된 바인딩
      요약이 평이한 한국어로 표시된다.
      — **검증**: SKILL.md 지시 정적 검증 + 도그푸드(실제 온보딩 1회, `_bindings/<id>.json` 생성 관찰).
- [x] **AC-2 (재설정)**: Given 기존 바인딩이 있는 설치에서, When 온보딩 진입점을 재실행하면,
      Then 현재 바인딩 값이 표시되고, 사용자가 선택한 항목만 수정 저장되며, 수정하지 않은
      항목은 보존된다.
      — **검증**: `mergeBinding` 유닛 + 재실행 도그푸드(항목 선택 수정·나머지 보존 관찰).
- [x] **AC-3 (설정 없이 워크플로 실행)**: Given 바인딩이 없는 설치에서, When 워크플로 스킬이
      페르소나·모델이 필요한 시점에 도달하면, Then 어떤 side-effect도 발생하기 전에 온보딩
      방법을 포함한 평이한 한국어 안내가 표시되고 워크플로는 기본적으로 진행하지 않는다.
      — **검증**: `resolveTier`/`resolveRole` 부재 사유 반환 유닛 + `binding-contract.md` D-4
      정적 + 도그푸드(무단 진행 0건 관찰).
- [x] **AC-4 (페르소나 미지원 런타임)**: Given 격리 서브에이전트 위임 능력이 없는 런타임에서,
      When 워크플로가 역할(예: critic) 수행을 요구하면, Then 현재 세션이 그 역할의 체크리스트를
      직접 수행하고 결과 보고에 "비독립(fallback)"이 명시되며, 워크플로는 중단되지 않는다.
      — **검증**: `binding-contract.md`의 FR-5(비독립 명시·중단 금지) 서술 정적 검증 + 도그푸드
      (가능한 환경 한정). 지침수준 행동 — 실소비는 051, 이번은 정적+도그푸드로 확인.
- [x] **AC-5 (런타임 격리)**: Given 런타임 A에서 바인딩을 설정한 상태에서, When 런타임 B에서
      워크플로를 실행하면, Then B는 A의 바인딩을 읽지 않고 B 자신의 바인딩 부재 규칙(AC-3)을
      적용한다.
      — **검증**: `loadBinding` 정확 일치 + traversal 가드 유닛(격리 회귀 4건) + 백업 격리 셸 31/0.

엣지 케이스:

- [x] **AC-6 (엣지 — 부분 설정)**: Given 일부 등급·역할만 설정된 바인딩에서, When 워크플로가
      설정된 항목과 미설정 항목을 모두 요구하면, Then 설정된 항목은 그대로 사용되고 미설정
      항목에만 부재 규칙(안내 또는 fallback)이 적용된다 — 바인딩 전체가 무효 취급되지 않는다.
      — **검증**: `validateBinding` 부분설정 유효 유닛(전체 무효화 시 실패하는 핀).
- [x] **AC-7 (엣지 — 잘못된 페르소나명)**: Given 온보딩 중 레지스트리에 없는 페르소나 이름이
      입력되면, When 저장을 시도하면, Then 저장 전에 경고와 재선택 유도가 표시되고 무효
      바인딩은 저장되지 않는다.
      — **검증**: `validateBinding` 무효 persona 거부 유닛 + SKILL.md 재선택 유도 지시 정적.
- [x] **AC-8 (엣지 — 빈 레지스트리)**: Given 페르소나 레지스트리가 비어 있는 설치에서, When
      온보딩을 실행하면, Then 역할 → 페르소나 단계는 사유 안내와 함께 건너뛰어지고 등급 → 모델
      설정은 정상 완료된다.
      — **검증**: 빈 레지스트리 유닛(F-2 규약 재사용) + SKILL.md 건너뜀 안내 지시 정적.
- [x] **AC-9 (엣지 — 추천 밖 모델명)**: Given 사용자가 추천 초안에 없는 모델 식별자를
      입력하면, When 확정하면, Then 그 값이 그대로 저장되고 "가용성은 검증하지 않는다"는
      고지가 표시된다.
      — **검증**: 자유 모델 식별자(F-12 형식 내) 저장 허용 유닛 + 미검증 고지 문구 도그푸드 관찰.
