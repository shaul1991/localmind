...
- **버전 확정(version stamping)**: package.json 버전 bump + CHANGELOG 버전 헤더 기입 —
  PR 머지 직전에 수행한다. git tag는 버전 확정에 포함되지 않는다 — **머지 후** 검증된
  main에 만든다(FR-5 ④). 셋(package.json·CHANGELOG 헤더·tag)은 항상 같은 버전 문자열을
  쓴다.
...
- [x] **FR-1 (CalVer 형식 규약)**: 버전 형식을 `YYYY.MM.MICRO`로 성문화한다 — `YYYY.MM` =
...
- [x] **FR-2 (관심사 분리 — 내용은 작업 중, 버전은 머지 직전)**: (a) 변경 내용 서술
      (CHANGELOG 항목 초안·PR 설명)은 **작업 중 작성해 PR에 포함**한다 — 버전 숫자 없이.
      (b) 버전 숫자 확정(package.json bump + CHANGELOG 버전 헤더)은 **PR 머지 직전**에
      수행하고, git tag는 **머지 후** 검증된 main에 만든다(FR-5 ④). 작업이 달을 넘겨도
      버전은 항상 실제 릴리스 시점의 연·월을 따른다.
      → goal: Objective(관심사 분리) / Problem 1(버전 시점 부정확)
- [x] **FR-3 (AGENTS.md 버전/릴리스 절 신설)**: AGENTS.md에 버전/릴리스 절을 신설해 FR-1·2·
...
- [x] **FR-4 (goal-ready "버전 미정" 편입)**: goal-ready 스킬 정본에 "버전은 여기서 정하지
      않는다 — 버전/릴리스 규약(AGENTS.md) 참조. 작업 시작 시점에는 변경 내용 서술만
      누적한다"를 명시한다.
      → goal: Objective(관심사 분리) / Constraints(스킬 정합)
- [x] **FR-5 (릴리스 절차 성문화)**: 릴리스 절차를 순서로 성문화한다 — ① main 직접 push
      금지(규약7 계승), ② PR 머지, ③ 머지된 main에서 버전 확정 커밋이 포함됐는지 확인,
      ④ `git fetch origin main --tags` 후 `git tag <CalVer> origin/main`(v 접두 없음, stale
      로컬 HEAD 오태그 방지) + push, ⑤ `gh release create <CalVer> --verify-tag`(원격 태그가
      이미 있어야 생성 — gh 자동 태그 생성 방지, CHANGELOG 해당 항목을 release notes로).
      비개발자도 따라 할 수 있게 각 단계에 무엇을 확인해야 하는지를 쓴다.
      → goal: Expected outcome(재질문 없는 릴리스) / Problem 2(절차 암묵)
- [x] **FR-6 (gh 계정·머지 검증 안전장치)**: 규약에 안전장치 2건을 편입한다 — (a) gh CLI로
...
- [x] **FR-7 (기존 문서 정합 — 모순 문구 정정)**: CHANGELOG 상단의 버전 체계 주석("문서
...
FR-1~7 충족, AC-1~5 정적 green. **self-review**: Claude critic(Fable) + **codex 교차**(gpt-5.5) 2라운드. 두 리뷰가 **중대 결함(tag 시점 자기모순)** 을 함께 잡음 — "버전 확정(+tag)을 머지 직전"이 절차(tag는 머지 후)와 모순 → **tag를 버전 확정에서 분리**(package.json+CHANGELOG는 머지 직전 PR 커밋, git tag는 머지 후 verified main)로 정정. codex가 gh/git 절차 오류도 추가 지적(공식 docs 인용): `git tag <CalVer> origin/main`(stale HEAD 오태그 방지)·`gh release create --verify-tag`(자동 태그 생성 방지)·`gh auth status`는 권한 검증 아님(활성 계정 표시) → 전부 반영. 재검 blocking 0. 전체 `npm test` **882 green** + build clean.

- **정적(AC-1~5)**: `workflow-docs.test.ts`의 053 describe 블록 — AGENTS.md 신설 절(`## 버전·릴리스` 슬라이스)·goal-ready·CHANGELOG 문구 assert(실파일 발췌). AC-5 구 문구 부재는 범위 한정(specs/ 제외, I-4) + 핀 유효성 실증(구 문구 삽입→RED→원복).
- **도그푸드(T5.1)**: `skills:deploy`로 goal-ready "버전 미정" 전파(성공 exit 0).
- **AC-6~9 후속 승계(정직 명시, 은폐 금지)**: AC-6(릴리스 수행)·AC-7(달 넘긴 작업)·AC-8(미머지 태그 차단)·AC-9(같은 달 2번째)는 **실제 릴리스 발생이 전제**라 052 AC-5·7과 동일하게 **다음 릴리스에서 관찰하는 후속 승계**다 — 정적 문구는 T4.1이 핀하나 행동 관찰은 052 DoD 이후에만 가능. 미체크로 두고 사유 부기.
