...
> **TL;DR** — 2025–2026 프론티어 랩 1차 자료·연구는 일관되게 "per-action 승인 → decision-by-exception + 위험 보정 감독"으로의 이행을 지지한다. 동시에 **모든 출처가 검증층의 '제거'가 아닌 '재배치'를 말한다** — 결정적 하드 가드레일과 구현 컨텍스트에서 격리된 독립 검증은 유지·강화. 근거: 강한 모델도 유창하고 자신 있는 언어로 오류를 내며(언어화 확신이 정확도를 체계적으로 상회하는 과신 — 질적 방향 라이브 확인, 구체 수치 1차 출처 미추적), 숙련자도 시간 압박에서 과신뢰한다.
> **누가/언제** — localmind 거버넌스 재보정(specs/202607202152)의 근거 문서, 2026-07-20 조사.
>
> **핵심 요약**
> - 확인 루프 밀도 → **완화**: 승인 프롬프트의 93%가 승인됨(정보 가치 낮음, Anthropic 실측)
> - 인터뷰 강제 밀도 → **완화**: 에이전트가 고복잡도 작업에서 인간 개입의 2배 이상 스스로 명확화 질문(50만 턴 분석, 복잡도 비례)
...
3. **[medium] 에스컬레이션은 양방향 창발.** 에이전트가 **최고 복잡도 작업에서** 인간 개입 빈도의 **2배 이상** 스스로 명확화 일시정지(복잡도 비례 — 저복잡도에선 낮음)(선택지 제시 35%·진단 21%·모호 명확화 13%) — 강제 인터뷰 밀도를 낮춰도 중요 분기 질문이 사라지지 않음. 한정: 단일 출처(Anthropic)·SW 도메인·의도 훈련의 결과. [measuring-agent-autonomy]
...
7. **[high] 유창함·확신은 신뢰 신호가 아니다.** 환각·조작을 정확한 출력과 동일하게 유창·자신 있게 전달. 프론티어 모델의 언어화 확신이 실제 정확도를 체계적으로 상회(과신, RLHF가 증폭 — 질적 방향은 arXiv 2410.09724 확인, '88% vs 79%' 구체 수치는 r1 렌즈⑤ Live-Verify에서 1차 출처 미추적으로 철회). 병리학 전문가 38/560건이 자기 정답을 버리고 AI 오답 채택, 2,784명 RCT에서 수정 노력이 크면 AI 오류 미교정 통과 — "유능해 보임"은 검증층 제거 근거가 될 수 없다. [arXiv 2509.08010·2410.09724·AI&Society]
...
| 출처 | 등급 | 역할 |
|---|---|---|
| https://www.anthropic.com/engineering/claude-code-auto-mode | T2 vendor-primary | 93% 승인률·분류기·reasoning-blind·차단 규칙 |
| https://www.anthropic.com/research/measuring-agent-autonomy | T2 vendor-primary | 자발 명확화 2배·auto-approve 추이 |
| https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf | T2 vendor-primary | HITL 트리거·위험 등급·optimistic execution |
| https://developers.openai.com/api/docs/guides/agents/guardrails-approvals | T2 vendor-primary | SDK 승인 인터럽트 |
| https://resources.anthropic.com/hubfs/2026%20Agentic%20Coding%20Trends%20Report.pdf | T2 vendor-primary | reviewing what matters·agentic QC |
| https://www.anthropic.com/research/how-ai-is-transforming-work-at-anthropic | T2 | 0–20% 완전 위임 |
