...
- [ ] 티어 판정 기록·중간 승격(하향 금지)·escalate-on-doubt(양 경계) (FR-3, AC-4·7·8)
...
- [ ] goal-ready 활성화 판정에 티어 확인 추가: Tier 0/1이면 4문서 미강제, lane 안내 (FR-2·3, AC-5)
- [ ] Tier 1 `change.md` 경로·최소 섹션 명문화, Tier 1도 TDD (OQ-1, AC-6·7)
- [ ] `templates/sdd/change.template.md` 스켈레톤 신설(GWT AC 힌트)
- [ ] `docs/workflows.md`에 세 티어 lane 사용자 설명(비개발자 한국어, AGENTS.md parity) (FR-9, AC-17)
...
- [ ] self-review 읽기 대상에 `matrix 행 = AC↔코드·evidence 지도` 추가(기존 diff 스코프 위에) (FR-4, AC-9)
- [ ] 독립성 가드레일 문구: "각 행 실제 코드 검증, matrix 상태 셀만으로 통과 금지" (FR-4, AC-10)
- [ ] 라운드 전량 재검증(승계·스킵 없음, per-round 독립성 보존) + within-run map 재사용·cross-session 금지 (FR-5·6, AC-11·12·13)
- [ ] critic.md에 matrix 지도 읽기 + 도장찍기 금지 반영 (FR-4, AC-10)
...
- [ ] 티어 인지 실행: Tier 1 in-session 1라운드, Tier 2 현행 격리 2라운드 (FR-3)
- [ ] 라운드 전환 시 전량 재검증(map만 재사용, verdict 승계·무효화-스킵 없음)을 §7A 인근 명문화 (FR-5, AC-11·12)
- [ ] round-to-round 무효화 판정 미도입 명시 + `202607181125` base-통합 재평가는 그 정본 소관으로 불변 (FR-5·7)
- [ ] 중간 티어 승격(하향 금지) (FR-3, AC-8)
...
- [ ] (RED 먼저) `tier-classification.test.ts`: AGENTS.md worked-example의 내부 정합·무모호성(AC-1~4)
- [ ] (RED 먼저) `workflow-policy.test.ts`: AC-5~13·15·17 문구 대조 + **AGENTS.md 로더 추가**(AC-5 대조 가능화)
- [ ] 배포 드리프트 0 확인 — `deploy.test.ts` 멱등 재배포 (AC-16)
- [ ] 기존 bounded-verification 계약 테스트 green 재확인 (AC-14) + 전체 스위트 회귀
- [ ] 도그푸드 — 각 티어 1건씩 실제 흐름 관찰(Tier 0 직접 / Tier 1 change.md / Tier 2 4문서)
...
- [ ] self-review clean 후 세 문서 검증 표기(`[x]` + 근거), 미충족은 사유 부기
- [ ] PR 본문 요약(두 축 + 202607181125 불약화 근거 + OQ-5 결정) 준비
...
|---|---|---|---|---|
| AC-1 | worked-example 정합(unit): Tier 0 예시 | AGENTS.md Tier0 트리거·예시 + 테스트 | Tier0 지정·config 제외 무모호, green | 대기 |
| AC-2 | worked-example 정합(unit): Tier 1 예시 | 트리거·예시 + 테스트 | Tier1 지정 무모호, green | 대기 |
| AC-3 | worked-example 정합(unit): 하드 신호 예시 | 하드신호 목록·예시 + 테스트 | 무조건 Tier2 지정, green | 대기 |
| AC-4 | worked-example 정합(unit): 양 경계 모호 예시 | escalate 규칙·경계 예시 + 테스트 | 양 경계 상향·하향 예시 없음, green | 대기 |
| AC-5 | 계약 대조(unit, AGENTS.md 로더 포함) | `has()`+AGENTS.md 로더 assertion | 티어 의식 매핑 전 surface 동일, green | 대기 |
| AC-6 | 계약 대조(unit): goal-ready/goal-impl | Tier1 TDD·테스트 생략 Tier0만 문구 | 문구 존재, green | 대기 |
| AC-7 | 계약 대조(unit): 티어 기록 규칙 | 티어·근거 기록 요구 문구 | 문구 존재, green | 대기 |
| AC-8 | 계약 대조(unit): goal-impl 승격 규율 | 중간 승격·하향 금지 문구 | 문구 존재, green | 대기 |
| AC-9 | 계약 대조(unit): self-review·critic | `matrix 행=AC↔코드 지도` 문구 | 문구 존재, green | 대기 |
| AC-10 | 계약 대조(unit): critic.md·self-review | "행마다 실제 코드 검증·도장찍기 금지" 문구 | 문구 존재, green | 대기 |
| AC-11 | 계약 대조(unit): 전량 재검증 | "라운드마다 전량 재검증·승계/스킵 없음" 문구 | 문구 존재, green | 대기 |
| AC-12 | 계약 대조(unit): 독립성·map은 통과근거 아님 | "verdict 미승계·map만 재사용·적극형 미도입" 문구 | 문구 존재, green | 대기 |
| AC-13 | 계약 대조(unit): map 재사용 범위 | within-run·cross-session map 금지 문구 | 문구 존재, green | 대기 |
| AC-14 | 회귀(unit): bounded-verification 테스트 실행 | 기존 스위트 결과(핀 문장 보존) | 전부 green 유지 | 대기 |
| AC-15 | 계약 대조(unit): Tier 2 규율 | 전 AC green·도그푸드·격리 critic·PR·Live-Verify 문구 | 문구 존재, green | 대기 |
| AC-16 | 배포(unit/통합): `deploy.test.ts` 멱등 | 배포 diff·드리프트 0 | 재배포 idempotent·드리프트 0, green | 대기 |
| AC-17 | 계약 대조(unit): `docs/workflows.md`↔AGENTS.md | human-doc parity assertion | 티어 설명 존재·드리프트 0, green | 대기 |
