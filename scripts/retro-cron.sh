#!/usr/bin/env bash
# 주기 자동 회고(스프린트 경계는 불규칙 — 기본 격주 월요일 대신 DOW·간격은 사용자 지정 권장, 032 OQ2) 등록 — crontab 에 'make retro' 예약을 넣어 주는 비개발자용 가이드.
# 호출: make retro-cron   (요일/시간 지정: DOW=1 HOUR=9 MIN=0 make retro-cron / 미리보기: DRY_RUN=1)
# 멱등: 이미 등록돼 있으면 새 시간으로 교체. 제거 방법은 마지막에 안내.
# (specs/032 FR-8 — backup-cron.sh 패턴 계승)
set -euo pipefail
...
HOUR="${HOUR:-9}"
MIN="${MIN:-0}"
MARKER="# localmind-retro"          # 우리 항목을 찾아 교체/삭제하기 위한 표식
LOG="\$HOME/localmind-retro.log"
...
say ""
say "$(b '주기 자동 회고(스프린트 경계는 불규칙 — 기본 격주 월요일 대신 DOW·간격은 사용자 지정 권장, 032 OQ2) 설정')을 시작합니다 — 매주 정해진 시각에 '$(b 'make retro')'로 워크플로우 회고 노트 노트를 만들어요."
say ""
...
LINE="$MIN $HOUR * * $DOW cd \"$PROJECT_DIR\" && PATH=\"$CRON_PATH\" ${CRON_ENVS}make retro >> $LOG 2>&1 $MARKER"
...
[ -n "$EXISTS" ] && warn "이미 등록된 자동 회고가 있어요 — 새 시간으로 교체합니다."
...
if ! confirm "이대로 자동 회고를 등록할까요?"; then
