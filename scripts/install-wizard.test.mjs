...
test("AC-3: 화이트리스트 id는 spawn을 호출하고 종료코드를 전달한다(fake spawn)", async () => {
  ...
  assert.match(spawnedWith.args[0], /scripts\/embedding-up\.sh$/);
...
test("AC-4: maskSecret은 원문을 반환하지 않고 끝 4자 힌트만", () => {
  const secret = "sk-emb-abcdef_SECRET_TAIL9999";
  const m = maskSecret("EMBEDDINGS_KEY", secret);
...
test("AC-4: resolveSecretKey는 알려진 항목만 env 이름으로", () => {
  assert.equal(resolveSecretKey("embeddings"), "EMBEDDINGS_KEY");
  // great-reduction r2: 백엔드 토큰 항목(claude/gemini)은 소비자 소멸로 화이트리스트에서 제거
  for (const bad of ["claude", "gemini", "PATH", "LITELLM_MASTER_KEY", "../x", "", null, 1]) {
...
test("AC-4: buildSecretResponse 응답 body에 원문이 절대 실리지 않는다", () => {
  ...
  const r = buildSecretResponse("embeddings", secret);
  assert.equal(r.status, 200);
  assert.equal(r.envName, "EMBEDDINGS_KEY");
...
test("AC-4: writeEnvVar는 격리 .env에 기록하고 원문을 응답에 두지 않으며 chmod 600", () => {
  ...
  const ret = writeEnvVar(envPath, "EMBEDDINGS_KEY", "NEWSECRET123");
  assert.equal(ret, true);
  const content = fs.readFileSync(envPath, "utf8");
  assert.match(content, /^EMBEDDINGS_KEY=NEWSECRET123$/m, "값이 교체돼야 함");
  assert.ok(!/EMBEDDINGS_KEY=old/.test(content), "옛 값은 남지 않아야 함");
  assert.match(content, /# comment/, "주석 보존");
  assert.match(content, /EXISTING=1/, "다른 키 보존");
  // 새 키 append
  writeEnvVar(envPath, "OTHER_KEY", "sk-XYZ");
  assert.match(fs.readFileSync(envPath, "utf8"), /^OTHER_KEY=sk-XYZ$/m);
  // 권한 600 (POSIX)
...
test("통합 AC-4(보안): 값 개행 인젝션은 거부되고 .env가 변조되지 않는다", async () => {
  ...
  try {
    const res = await fetch(`http://127.0.0.1:${port}/api/secret`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ key: "embeddings", value: "harmless\nLITELLM_MASTER_KEY=ATTACKER" }),
    });
    ...
    assert.ok(!/EMBEDDINGS_KEY/.test(env), "거부됐으므로 기록도 없어야 함");
  } finally {
    server.close();
...
test("단위(보안): writeEnvVar는 개행 든 값을 거부(throw)한다", () => {
  ...
  assert.throws(() => writeEnvVar(envPath, "EMBEDDINGS_KEY", "x\nB=2"), /줄바꿈/);
...
test("통합 AC-4: POST /api/secret은 원문을 응답에 싣지 않고 격리 .env에 기록한다", async () => {
  ...
  try {
    const res = await fetch(`http://127.0.0.1:${port}/api/secret`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ key: "embeddings", value: secret }),
    });
    ...
    // 서버측 .env에는 기록됨
    assert.match(fs.readFileSync(envPath, "utf8"), /^EMBEDDINGS_KEY=AIzaSy_INTEG_RAW_7777$/m);
  } finally {
    server.close();
    if (prev === undefined) delete process.env.LOCALMIND_ENV_FILE;
    else process.env.LOCALMIND_ENV_FILE = prev;
...
test("COMMANDS는 파괴적 명령을 포함하지 않는다", () => {
  ...
  assert.deepEqual(Object.keys(SECRET_KEYS).sort(), ["embeddings"]);
});

// ── 회귀 방지(great-reduction r2 B-NEW): 화이트리스트가 가리키는 스크립트는 실존해야 한다 ──
// 구 스택 스크립트 삭제 후 COMMANDS.up이 유령 스크립트를 spawn(exit 127)한 회귀의 재발 방지(great-reduction).
test("COMMANDS의 모든 script 파일이 scripts/ 아래에 실제로 존재한다", () => {
  const root = path.resolve(path.dirname(new URL(import.meta.url).pathname), "..");
  for (const [id, c] of Object.entries(COMMANDS)) {
    const p = path.join(root, "scripts", c.script);
    assert.ok(fs.existsSync(p), `COMMANDS.${id} → ${c.script} 이 존재해야 함(유령 스크립트 금지)`);
  }
});
