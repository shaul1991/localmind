...
- `deploy`(로컬 실행) 한 단계로 그 기기의 Claude·Codex 글로벌 표면과 현재 repo의 규칙 파일이
  정본과 일치하게 갱신된다.
- 규칙 파일 없던 repo도 그 repo에서 배포하면 그 프로젝트 overlay가 담긴 `AGENTS.md`(+`CLAUDE.md`
  스텁)를 얻는다(base는 글로벌 표면이 주입 — repo 중복 없음, D8).
...
- [x] 규칙 파일이 없던 repo(overlay 있는 프로젝트)에서 배포 시 그 overlay가 담긴 `AGENTS.md`와
  `@AGENTS.md` 스텁 `CLAUDE.md`가 생성된다(base는 글로벌; Claude·Codex 둘 다 글로벌 base + repo overlay).
