...
- **Tier 1(작음)** — 영향 범위가 좁은 국소적인 변경입니다. 아래 Tier 2 목록(새 개념·API/스키마
  계약·인증/보안·마이그레이션·데이터 모델·전역 상태나 직렬화 형식·여러 영역에 걸친 변경) 중
  어디에도 해당하지 않습니다. 이때는 4개짜리 문서 대신 `change.md` **한 장**만 씁니다 — 왜(why)·
  무엇을(what)·검증 기준(AC)·이 등급으로 판단한 근거를 짧게 적습니다. 문서가 가벼워져도
  **검증(테스트)은 생략하지 않습니다** — 테스트를 아예 안 써도 되는 것은 Tier 0뿐입니다. 검토도
  같은 세션이 스스로 결함을 찾아보는 가벼운 1회 자기검증으로 충분합니다.
- **Tier 2(실질적)** — 다음 중 **하나라도** 해당하면 변경 자체는 작아 보여도 **무조건 이
  ...
  변경. 지금까지 이 문서가 설명해 온 goal/spec/plan/tasks 네 문서와, 별도로 분리된 검토자가 최대
  2회까지 다시 확인하는 절차를 그대로 거칩니다.
