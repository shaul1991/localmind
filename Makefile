...
.PHONY: backup
backup: ## 메모리 export + 노트 백업 repo 커밋·푸시 (BACKUP_DIR; 최초 1회는 make backup-init; 개인 설정 파일도 함께: BACKUP_EXTRA_FILES=)
	...
	if git -C "$(BACKUP_DIR)" remote | grep -q .; then git -C "$(BACKUP_DIR)" push -q -u origin HEAD && echo "✓ push → $(BACKUP_DIR)"; \
...
restore: ## 백업에서 복원: 노트 repo pull/clone → memory-import → 재인덱싱 (스택 떠 있어야 함)
	...
	@echo "→ 노트 재인덱싱"; k=$$(grep -E '^LITELLM_MASTER_KEY=' .env 2>/dev/null | head -1 | cut -d= -f2-); \
	NOTES_DIR="$${NOTES_DIR:-$(BACKUP_DIR)}" LITELLM_MASTER_KEY="$$k" npm run reindex \
