...
```
  ...
  [홈서버 = 상시 켜둔 맥/리눅스]
      ...
      │                      ├─ HTTP → 게이트웨이 :8787 / 메모리 :8767
      │                      └─ 로컬 파일 → NOTES_DIR (.md 노트), ~/.localmind/agents
      └─ docker compose (게이트웨이·LiteLLM·메모리) + (선택) make ui-bg :8788
```
...
```bash
docker compose up -d          # 게이트웨이(:8787)·LiteLLM(:4000)·메모리(:8767)를 백그라운드로
make ui-bg                    # (선택) 모니터링 웹 UI :8788 백그라운드
make mcp-install              # 이 서버의 Claude Code에 localmind MCP 등록(~/.claude 설정)
make health                   # 스택 상태 확인
```
...
- `make mcp-install`로 이미 등록돼 있으므로 Claude Code가 뜰 때 localmind 도구(`search_notes`,
  `capture_note`, `ask_brain`, `recall`, `remember`, `deploy_agents` 등)가 붙는다.
- 노트·메모리·에이전트는 **홈서버의** NOTES_DIR / 메모리 서비스 / `~/.localmind/agents`를 쓴다 —
...
- [ ] 홈서버 서비스 포트(8787/4000/8767/8788)는 **루프백(127.0.0.1)** 바인딩 유지 — 인터넷에 열지 않는다.
- [ ] 원격 접근은 **Tailscale 사설망**으로만. SSH는 **키 인증**(비밀번호 로그인 비활성 권장).
- [ ] `.env`의 `LITELLM_MASTER_KEY` 등 비밀은 서버에만. 노트 저장소·백업에 절대 커밋 금지.
...
2. **임베딩 엔진을 다시 켠다** — 둘 중 하나를 고른다:
   ...
   - **`make embed`(LiteLLM 경유)**: Ollama를 직접 바라보지 않고 기존 게이트웨이 경로를 그대로
     쓰고 싶으면 이쪽을 쓴다.
