...
## 구현 결정 (self-review 반영)

- **테스트 glob = `src/*.test.ts` (최상위만)**: `src/**/*.test.ts`는 POSIX sh(globstar
  off)에서 `src/*/*.test.ts`로 해석돼 최상위 파일을 놓치고, `src/*/*.test.ts`를 추가하면
  Node 20(glob 미지원)이 미매치 리터럴을 파일 경로로 오인해 실패한다. 단일 `*` 패턴만이
  Node 20/22/24 + sh에서 안전하다. **따라서 테스트 파일은 반드시 `src/` 최상위에 둔다**
  (하위 디렉토리 `src/util`·`src/backends`·`src/routes` 아래 테스트를 두면 CI가 조용히
  건너뛰므로 금지). 하위 테스트가 필요해지면 test 스크립트를 tsx 러너 스크립트로 바꿔
  파일 탐색을 코드로 처리하는 별도 작업으로 다룬다.
- **셸 테스트 루프에 `shopt -s nullglob`**: `scripts/*.test.sh`가 하나도 없을 때 리터럴
  glob으로 거짓 실패하는 것을 방지.
