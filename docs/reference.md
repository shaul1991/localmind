...
```python
from openai import OpenAI
c = OpenAI(base_url="http://localhost:4000/v1", api_key="<LITELLM_MASTER_KEY 값>")  # .env 참조(설치 시 자동 생성)
...
| `LITELLM_MASTER_KEY` | (설치 시 자동 생성) | 게이트웨이 인증 키 — `make init-env`가 임의 값 생성, `make secrets`로 확인 |
