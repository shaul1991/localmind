...
- [ ] `gemini-3.5-flash` 요청이 OpenAI 호환(`/v1/chat/completions`) 경로에서 스트리밍 응답을
      반환한다. *어댑터 스트리밍·라우팅은 단위 검증(green). **실 200 응답은 라이브 미검증** —
      키 부재로 도그푸드 보류(AC-1).*
- [ ] Gemini 응답에 입력·출력 토큰 usage가 0이 아닌 값으로 집계된다. *어댑터 매핑은 단위 검증.
      **beta가 실제로 usage를 싣는지는 라이브 미검증**(AC-5·Phase 0).*
- [ ] 2턴 이상 대화에서 문맥이 유지된다. *메커니즘 코드 확인(full-history). **실 2턴은 라이브
      미검증**(AC-6).*
...
- [ ] Phase 0 스파이크 결과가 문서(spec/plan)에 반영되어, 미검증 가정이 확정 또는 Open
      question으로 정리됐다. *Phase 0 자체가 키 부재로 보류 — 미실행. 단, ④(멀티턴 흐름)는
      코드로 확인해 문서에 반영. ①②③⑤는 실키 대기.*
