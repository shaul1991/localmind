...
|---|---|---|---|---|
| AC-1 | 계약(unit) — AGENTS.md 텍스트 | 테스트 로그 | 기존 핀 유지 + 조건부 문구 존재 + 구 무조건 문구 제거, green | |
| AC-2 | 계약(unit) — SKILL 텍스트 | 테스트 로그 | 선언 문법·3조건·표기·critic 검증·저비용 예외 문구, green | |
| AC-3 | 계약(unit) — SKILL 텍스트 | 테스트 로그 | 기존 핀 유지 + 정본 참조 연결, green | |
| AC-4 | 계약(unit) — 전체 스위트 | 전체 실행 로그 | AC-11·13 불변·AC-12 교체·신규 핀 전부 green | |
| AC-5 | 단위 — 판정 함수 5케이스 | 테스트 로그 | 승계 1·재실행 4(사유 포함), green | |
| AC-6 | 단위 — 집계·렌더 | 테스트 로그 | 승계 건수 집계·부재 무영향, 기존 green 유지 | |
