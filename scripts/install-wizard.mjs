 ...
 * 설치 로직은 재구현하지 않고 기존 up.sh·mcp-install.sh·doctor.sh를 자식 프로세스로 호출한다(FR-6).
 */
...
export const COMMANDS = {
  up: { script: "up.sh", label: "스택 기동", streams: true },
 ...
 * claude-token.sh(키 라인 교체)·up.sh(chmod 600, specs/015 FR-9) 관례 계승.
 */
  ...
  return new Promise((resolve) => {
    ...
    try {
      child = spawnFn("bash", [scriptPath], {
        cwd: ROOT,
        // LOCALMIND_STREAM: up.sh 등이 로그 소각을 풀고 개행 진행을 내도록(단계 0c 결론).
