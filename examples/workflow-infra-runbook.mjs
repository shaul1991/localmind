...
console.log(`서버: ${SERVER}  노트: ${notesDir()}`);

// 1) 런북/스펙 적재 (그 서버의 NOTES_DIR에 .md로)
await capture(
  `${SERVER}: PostgreSQL 16, 32GB RAM, 일일 백업 02:00(UTC), 커넥션 풀 max=100, 모니터링 Grafana.`,
  `${SERVER} 런북`,
);

// 2) 장애 기록 (진화 기억)
await remember(`${SERVER} 장애 2026-06-27 03:12: 커넥션 풀 고갈로 5분 지연. 조치: max 100→200, 슬로우쿼리 인덱스 추가.`);

// 3) 그 서버 한정 RAG
const { answer, sources } = await askBrain(`${SERVER}의 스펙과 백업 정책을 정리해줘`);
console.log("\n## 런북 RAG\n" + answer);
console.log("출처:", sources.join(", "));

console.log("\n💡 워크플로우: 서버별 노트 폴더(NOTES_DIR) → 런북 capture_note → 장애 remember → 회고/온콜에 ask_brain (그 서버만)");
