...
- **회고 노트의 self-review 라운드 집계 표**: `make retro`가 만드는 노트에는 spec별로 아래
  컬럼이 있습니다.
  - **라운드**: 그 spec을 완료하기까지 자동 self-review를 몇 번 돌렸는지(최대 2회 상한).
  - **총 blocker**: 라운드를 통틀어 발견된 치명·중대 결함 수.
  - **최종 completion**: 마지막 라운드가 clean으로 끝났는지, blocker가 남았는지.
  - **duration(분)**: self-review에 걸린 총 소요 시간(분).
  - **형태**: 라운드마다 어떤 방식으로 검토했는지(예: 격리 리뷰어 단독, 렌즈 병렬 등).
  - **승계**: 이전 라운드의 실행 결과(evidence)를 조건부로 재사용한 건수(없으면 `-`).
...
**현재 packaged workflow catalog**:

| 논리 ID | 목적 | activation | side effect |
|---|---|---|---|
| `deep-research` | 공식·1차 근거 중심의 심층 조사와 비판적 검수를 채팅으로 보고 | `explicit` | `report-only` |
| `research-evidence-pack` | 완료한 조사와 ledger를 정확히 다섯 파일로 저장·구조 검증 | `explicit` | `docs-only` |
| `goal-ready` | 개방형 요구를 조사해 goal/spec/plan을 만들고 확인을 받음 | `intent` | `docs-only` |
| `goal-impl` | 준비된 SDD를 TDD로 구현·self-review·문서 체크·clean 완료 | `explicit` | `mutating` |
| `sdd-self-review` | 구현 결과를 적대적으로 review하고 완료 가능 여부 판정 | `delegated-or-explicit` | `report-only` |
| `localmind-binding` | 설치별 실행 등급→모델·역할→페르소나 바인딩 설정 | `explicit` | `mutating` |
| `localmind-rules` | localmind 거버넌스 규칙 저작·관리 | `explicit` | `mutating` |
...
|---|---|---|---|
| `deep-research` | `/deep-research <topic>` | `$deep-research <topic>` | auto skill 또는 생성된 `/deep-research <topic>` wrapper |
| `research-evidence-pack` | `/research-evidence-pack <confirmed-path>` | `$research-evidence-pack <confirmed-path>` | 공용 Agent Skill만 배포; generated wrapper는 `skipped-dependency` |
| `goal-ready` | `/goal-ready` | `$goal-ready` | auto skill 또는 `/goal-ready` wrapper |
| `goal-impl` | `/goal-impl <prefix>` | `$goal-impl <prefix>` | auto match 또는 `/goal-impl <prefix>` wrapper + 새 확인 |
| `sdd-self-review` | `/sdd-self-review` | `$sdd-self-review` | auto skill 또는 `/sdd-self-review` wrapper |
| `localmind-binding` | `/localmind-binding` | `$localmind-binding` | auto skill 또는 `/localmind-binding` wrapper |
| `localmind-rules` | `/localmind-rules` | `$localmind-rules` | auto skill 또는 `/localmind-rules` wrapper |

Codex 공식 계약에는 bare `/name` 등록이 없어 `$name`으로 표기합니다 — 존재하지 않는 slash
command를 약속하지 않습니다. 따라서 Codex의 bare `/deep-research`는 공식 문법이 아니며 사용하거나
권장하지 않습니다. deprecated Custom Prompts와 `/prompts:deep-research` 경로도 사용하지 않습니다.

**활성화 권한과 enforcement 수준(정직하게 구분)**:

- `goal-impl`는 코드 변경·commit·push·CI까지 갈 수 있어, **runtime이 보증한 명시 호출 +
  정확한 spec 폴더 프리픽스**(생성 시점 timestamp `YYYYMMDDHHmm`(초까지 확장 시 14자리) 또는
  레거시 3자리 번호 — 또는 provenance 없는 런타임에서 바로 앞 턴의 새 확인)가 있을 때만
...
- `deep-research`는 `explicit`·`report-only`입니다. 명시 호출과 조사 주제가 확인되기 전에는
  source lookup·fan-out을 시작하지 않으며, 기본 산출물은 외부 상태 변경이 아닌 채팅 보고입니다.
  행동 정본은 [canonical skill](../templates/skills/deep-research/SKILL.md)과
  [research contract](../templates/skills/deep-research/references/research-contract.md)입니다.
- `research-evidence-pack`은 `explicit`·`docs-only`입니다. 사용자가 확인한 새 디렉터리에만
  `report.md`·`sources.jsonl`·`evidence.jsonl`·`claims.jsonl`·`run-manifest.json`의 정확히 다섯
  파일을 쓰며, 기존 경로를 덮어쓰거나 HOME/Documents를 추정하거나 결과를 자동으로 열지 않습니다.
  동봉된 Python validator는 구조·참조 무결성만 검사하고 주장 자체의 진실성을 대신 판정하지 않습니다.
  실행 파일을 포함해 텍스트 wrapper로 자급할 수 없으므로 Gemini generated wrapper는
  `skipped-dependency`이며, 공용 Agent Skill의 실제 자동 발견 여부는 runtime/workspace 해석에 따릅니다.
- Gemini CLI에는 공식 실행 전 hook이 존재합니다. 다만 LocalMind가 생성하는 command
  wrapper는 그 hook을 설치하거나 등록하지 않으므로, 현재 확인 절차는 **지침 수준
  (instruction-level) 가드**입니다. "도구 호출 0회"를 런타임이 기술적으로 보장한다고
  과장하지 않으며, Gemini의 auto 활성화도 consent 게이트를 우회하지 않습니다.
...
`/goal {NNN}` 사용자는 같은 절차를 `goal-impl {prefix}`(위 표의 런타임별 문법)로 씁니다. (이름
변경 이력은 specs/051, 프리픽스 전환은 specs 폴더 규약 참조.)

### `goal-impl` 검증은 강하게, 자동 반복은 유한하게

`goal-impl`의 self-review는 구현한 세션과 분리된 **격리 reviewer**를 우선합니다. review 대상인
코드·계약·필수 evidence 한 세대를 **candidate**라고 하고, 같은 candidate를 reviewer 여러 명이
검수해도 findings를 합친 **merged report 하나가 round 하나**입니다. reviewer 수나 테스트 횟수
자체가 round를 늘리지는 않으며, finding을 고쳐 candidate가 바뀐 뒤 새 merged report를 만들 때
다음 round가 됩니다.

- 자동 self-review는 최대 **2 round**입니다. round 1이 clean이면 곧바로 끝내고, blocker를 고친
  candidate만 round 2에서 자동 재검합니다.
- round 2에도 blocker·미충족 AC·실패한 필수 테스트가 남으면 **성공이 아니라 중단**입니다. 남은
  문제와 수정·테스트 상태, 다음 review 목적을 사용자에게 보여준 뒤 멈춥니다.
- 추가 review는 그 보고를 본 사용자의 **fresh round approval**이 있어야 하며, 승인 1개는 바로
  다음 round 1개만 허용하고 소진됩니다. 과거·포괄·암묵 승인이나 승인 재사용은 인정하지 않습니다.
- 이 상한은 구현 self-review가 끝없이 반복되는 일을 막는 운영 경계입니다. `goal-ready`의 문서
  critic이나 `deep-research`의 조사 final critic은 각자 정본 계약을 따르며 이 2-round 예산의
  대상이 아닙니다.

최종 critic의 강도, TDD·필수 dogfood·전 AC green 같은 품질 조건은 그대로입니다. 자동 예산이
끝났다는 사실만으로 완료하거나 commit·push 단계로 넘어가지 않습니다.

**소유권·안전**: 정본은 노트 폴더 `skills/`(백업 자동 포함)이고 각 런타임 배포본은
파생입니다 — 고칠 때는 정본에서. 이름 결합 managed marker가 있는 산출물만 갱신·정리(prune)
하고, 직접 만든 스킬/명령(마커 없음)은 건드리지 않습니다. packaged catalog의 논리 ID는 **예약 이름**이라,
...
```bash
{ cat specs/<prefix>-*/spec.md; git diff; } | localmind-review   # 직접 실행도 가능
```
