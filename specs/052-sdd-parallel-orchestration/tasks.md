...
- [x] **T4.1** F-7 패턴(packaged 문서 직접 읽어 문구 assert, → plan F-7)으로 케이스 추가:
...
- [x] **T4.2** packaged 전수 중립성 스캔에 신설 `references/tasks-format.md` 포함 clean 확인
...
- [x] **T5.1** `AGENTS.md`에 tasks 형식 규약 포인터 1~2줄(외과적 — → plan 영향 모듈).
- [x] **T5.2** `docs/workflows.md`에 사람용 병렬 오케스트레이션 설명·구체 예시 — 스캔 밖 채널
...
- [x] **B-L3** L3(Phase 4 ∥ Phase 5) 완료 후 **메인이** 전체 `npm test` green + `npm run build`
