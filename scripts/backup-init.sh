...
# ── 백업 위치 고지 (informed consent) ───────────────────────────
say "$(b '📍 백업 위치 안내 — 시작 전 꼭 확인하세요')"
say "  이 백업은 localmind의 $(b '내 노트·메모리 전체')를 $(b '내 GitHub 개인 계정')의 비공개 저장소에 올립니다."
say "  • 비공개라 남들은 못 보지만, $(b '회사가 아니라 내 개인 계정')에 저장됩니다."
say "  • $(b '업무·회사·고객 데이터')를 담았다면, 개인 계정 백업이 회사 데이터 정책에 어긋나지 않는지 먼저 확인하세요."
say "  • 권장: 회사 데이터는 조직이 지정한 저장소에, 개인 백업엔 개인 데이터만."
if ! confirm "계속 진행할까요?"; then
  say "  중단했습니다. 필요하면 정리 후 다시 '$(b 'make backup-init')' 을 실행해 주세요."
  exit 0
fi
say ""
