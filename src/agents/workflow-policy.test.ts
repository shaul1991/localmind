...
describe("tier-contract: AC-12 — per-round 독립성 보존 + map은 통과근거 아님 + 조건부 evidence 승계(specs/202607210545)", () => {
  it("sdd-self-review가 map만 재사용·verdict 스킵 금지·조건부 evidence 승계를 명시하고 구 무조건 문구는 부재한다", () => {
    has(
      "sdd-self-review",
      "재사용되는 것은 **검증 결과가 아니라 map뿐**이다",
      "일부 행만 재검증하는 verification skip(verdict 스킵)은 여전히 금지한다",
      "hermetic(결정적)·고비용",
      "재실행이 기본",
      "승계: rN@<candidate 7자 SHA>",
      "cross-session 승계는 여전히 금지",
    );
    assert.ok(!flatBody("sdd-self-review").includes("round-to-round 무효화-스킵(적극형)은"), "구 무조건 문구가 sdd-self-review에 남음");
  });
  it("goal-impl이 map은 통과 근거가 아님·verdict 스킵 금지·evidence 승계는 sdd-self-review 정본 참조를 명시하고 구 무조건 문구는 부재한다", () => {
    has(
      "goal-impl",
      "map은 \"어디를 보라\"만 정할 뿐 통과 근거가 아니다",
      "일부 행만 재검증하는 verification skip(verdict 스킵)은 이 워크플로에 여전히 금지한다",
      "evidence 실행 결과의 조건부 승계",
      "sdd-self-review SKILL \"승계 절차\" 절이 정본이다",
    );
    assert.ok(!flatBody("goal-impl").includes("적극형"), "구 무조건 문구(적극형)가 goal-impl에 남음");
  });
});

describe("hermetic-evidence-reuse: AC-1 — AGENTS.md(루트+스캐폴드) 조건부 승계 문구 + 구 무조건 문구 부재(specs/202607210545)", () => {
  it("루트 AGENTS.md와 templates/sdd/AGENTS.md 스캐폴드 양쪽에 3조건·보수 기본·출처 표기·cross-session 금지 불변 문구가 존재한다", () => {
    for (const relPath of ["AGENTS.md", "templates/sdd/AGENTS.md"]) {
      hasIn(
        relPath,
        "일부 행만 재검증하는 verification skip(verdict 스킵)은 여전히 금지한다",
        "hermetic(결정적)·고비용·수정 diff와 선언 의존의 교집합이 공집합",
        "승계: rN@<candidate 7자 SHA>",
        "선언 부재·조건 애매·저비용(전체 스위트·preflight)은 재실행(보수 기본)",
        "cross-session 금지 불변",
      );
    }
  });
  it("양쪽 모두 구 '적극형은 도입하지 않는다' 무조건 문구가 부재한다", () => {
    for (const relPath of ["AGENTS.md", "templates/sdd/AGENTS.md"]) {
      assert.ok(!flatFile(relPath).includes("적극형)은 도입하지 않는다"), `${relPath}에 구 무조건 문구가 남음`);
    }
  });
});
