...
3. 선행 문맥을 확인하고 T1/T2 우선 source strategy를 세운 뒤, 독립적인 질문만 read-only lane으로
   나눕니다.
4. 한 run 안에서 `R-001`·`S-001`·`E-001`·`C-001` 형식의 안정 ID를 부여하고, claim별 atomic
   evidence ledger에 URL·권위·날짜·지지/반박·확인 상태를 연결하며 상충 근거를 숨기지 않습니다.
   고정 출처 개수 대신 근거의 직접성·권위·독립성·중요 주장 커버리지·상충 해소 상태로 충분성을
   판정합니다.
5. 모든 lane이 끝난 뒤 synthesis → final critic 순서로 검수하고, TL;DR·scope/기준일·핵심 발견·
   근거·상충/한계·권고·Open questions·실행 투명성 순서로 보고합니다.

조사가 한 세션을 넘기면 checkpoint에 완료·미완료 질문, source/evidence/claim ID, 상충 근거,
다음 행동을 남겨 같은 run을 중복 조사 없이 이어갑니다.
...
### Evidence pack — 명시적으로만 파일 저장

`deep-research`의 기본값은 계속 `report-only`입니다. 이미 완료한 보고와 ledger를 파일로 보존해야 할
때만 별도 logical ID `research-evidence-pack`을 명시 호출합니다.

| 런타임 | 실제 호출 |
|---|---|
| Claude Code | `/research-evidence-pack <confirmed-path>` |
| Codex | `$research-evidence-pack <confirmed-path>` |
| Gemini CLI | 공용 Agent Skill만 배포; generated wrapper는 `skipped-dependency` |

사용자가 확인한 **존재하지 않는 새 디렉터리**에만 아래 정확히 다섯 파일을 원자적으로 완성합니다.
...
기존 경로·파일은 덮어쓰지 않고 HOME/Documents 같은 위치를 추정하지 않으며, 저장 뒤 브라우저나
편집기를 자동으로 열지 않습니다. 동봉 Python 3.9+ 표준 라이브러리 validator는 ID·필수 필드·참조
무결성·claim coverage를 읽기 전용으로 확인할 뿐, 주장의 사실성을 대신 보증하지 않습니다.
이 validator 실행 파일을 텍스트-only command wrapper에 안전하게 내장할 수 없으므로 Gemini generated
wrapper는 생성하지 않습니다. 공용 Agent Skill target에는 배포되지만 실제 auto-discovery는 해당
runtime과 workspace의 skill resolution에 따라 달라지며, 확인 전에는 지원된다고 단정하지 않습니다.
...
## 변경 등급(Tier) — 내 변경 크기에 맞는 lane 고르기

> **결론** — localmind는 모든 변경에 같은 무게의 절차를 강제하지 않습니다. 작업을 시작하기 전에
> **변경 등급(Tier)**을 먼저 정해, 오타 수정 같은 작은 일에는 문서·검토를 생략하고 API·보안·
> 마이그레이션처럼 파장이 큰 일에는 지금까지 설명한 4문서·격리 검토를 그대로 유지합니다. 이
> 판정은 별도 프로그램이 자동으로 매기는 것이 아니라, `AGENTS.md`에 적힌 규칙을 세션이 매번
> 읽고 사람이 검토하듯 적용하는 것입니다.

세 등급 중 하나로 갈립니다.

- **Tier 0(트리비얼)** — 오타·주석·문서 문구 수정, 포매팅 정리처럼 **동작이 절대 바뀌지 않는 게
  분명한** 변경입니다. 문서도 검토도 만들지 않고 곧바로 고칩니다. 단, 타임아웃·요금제·모델명 같은
  **설정값 변경은 여기 포함되지 않습니다** — 값 하나가 동작을 바꿀 수 있기 때문입니다.
- **Tier 1(작음)** — 영향 범위가 좁은 국소적인 변경입니다. 아래 Tier 2 목록(새 개념·API/스키마
  계약·인증/보안·마이그레이션·데이터 모델·전역 상태나 직렬화 형식·여러 영역에 걸친 변경·비가역성)
  중 어디에도 해당하지 않습니다. 이때는 4개짜리 문서 대신 `change.md` **한 장**만 씁니다 — 왜(why)·
  무엇을(what)·검증 기준(AC)·이 등급으로 판단한 근거를 짧게 적습니다. 문서가 가벼워져도
  **검증(테스트)은 생략하지 않습니다** — 테스트를 아예 안 써도 되는 것은 Tier 0뿐입니다. 검토도
  같은 세션이 스스로 결함을 찾아보는 가벼운 1회 자기검증으로 충분합니다. 단, 결정적 테스트로 전체를
  검증할 수 있고 되돌리기도 쉬운 변경일수록 이 등급으로 보는 근거가 더 튼튼해집니다 — 반대로 아래
  Tier 2 목록에 하나라도 해당하면 이 근거는 무효이고 무조건 Tier 2입니다.
- **Tier 2(실질적)** — 다음 중 **하나라도** 해당하면 변경 자체는 작아 보여도 **무조건 이
  등급**입니다: 새로운 도메인 개념 도입, API·데이터 스키마·이벤트 같은 **계약** 변경, 인증·보안
  표면, **마이그레이션**, 데이터 모델 변경, 전역 상태나 직렬화 형식 변경, 여러 영역에 걸친
  변경, 외부에 발행하거나 데이터를 되돌릴 수 없이 파괴하거나 되돌릴 수 없는 마이그레이션을 거치는
  **비가역성**을 가진 변경. 지금까지 이 문서가 설명해 온 goal/spec/plan/tasks 네 문서와, 별도로
  분리된 검토자가 최대 2회까지 다시 확인하는 절차를 그대로 거칩니다.

판단이 애매할 때는 **항상 더 무거운 등급 쪽으로** 올립니다(Tier 0인지 1인지 애매하면 1로, 1인지
2인지 애매하면 2로). 가벼운 쪽으로 임의로 낮춰 잡지 않습니다. 작업 도중 API 변경이나 보안 이슈처럼
무거운 요소가 새로 드러나면 그 자리에서 Tier 2로 올리고 그 사실을 알립니다 — 반대로 이미 정한
등급을 도중에 낮추는 일은 없습니다. 어떤 등급으로 판단했는지와 그 근거는 매번 결과물에 남습니다
(Tier 1은 `change.md`의 "티어 근거" 항목, Tier 2는 goal/spec 문서와 최종 검토 보고).
...
### 6. critic이 보기 전에 형식을 먼저 자동 확인하고, 여러 리뷰어가 나눠 보고, 비싼 증거는 조건부로 재사용합니다

**preflight** — 저장소가 결정적 사전 검사(`npm run review:preflight -- specs/<spec 폴더>`)를
제공하면 critic이 내용을 보기 전에 먼저 돌립니다. evidence 경로 형식·diff 형식·merged report
필수 항목·matrix 전체 대응 여부 같은 기계적 실수를 미리 잡아냅니다. **preflight 통과는 형식
검사일 뿐**이며 어떤 AC도 그것만으로 통과시키지 않습니다 — 내용 검증은 여전히 critic이 합니다.

**렌즈 병렬** — 격리 리뷰가 가능하면 추적성·커버리지·정확성·단순성/보안·사실 정확성 다섯 점검
축을 리뷰어 여러 명이 동시에 나눠 볼 수 있습니다. 몇 명이 보든, 같은 candidate에 대한 모든
findings를 합친 보고서 한 장이 review round 한 번입니다 — round를 세는 규칙은 병렬로 보든 한
명이 순서대로 보든 그대로입니다.

**evidence 승계** — round가 바뀌어도(blocker 수정 후 새 candidate) 리뷰어의 판단은 항상 새로
내립니다. 다만 이미 실행해 둔 값비싼 증거는 ① 이번에 고친 파일과 그 AC가 의존한다고 밝힌
파일이 겹치지 않고 ② 그 증거가 다시 실행해도 같은 결과가 나오는 결정적인 것이고 ③ 다시
실행하면 전체 테스트보다 훨씬 비쌀 때, 이 세 조건을 모두 만족해야만 재사용할 수 있습니다.
재사용한 경우 "승계: r1@<커밋 7자리>"처럼 출처를 반드시 남기며, 세 조건 중 하나라도 애매하면
기본값은 다시 실행하는 쪽입니다. 자세한 규칙은 `sdd-self-review` 스킬 문서
(`templates/skills/sdd-self-review/SKILL.md`)를 참고하세요.
