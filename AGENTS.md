...
- 새 작업 착수 시 **"이 변경이 second-brain 사용자(디바이스 간 동일 사용)에게 무엇을 주는가?"**
  에 답한다 — 답이 없으면 메타 확장으로 간주하고 보류한다(티어 판정 앞의 선행 게이트).
...
- **Tier 2 — 실질적:** 아래 **하드 신호 중 하나라도** 해당하거나 blast-radius가 넓은 변경. 크기가
  ...
  - **하드 신호:** 신규 도메인 개념 · 계약(API/스키마/이벤트) 변경 · 인증·보안 표면 · 마이그레이션 ·
    데이터 모델 변경 · 전역 상태·직렬화 형식 변경 · 크로스커팅 변경 · 비가역성(외부 발행/데이터
    파괴/비가역 마이그레이션).
- **escalate-on-doubt (양 경계):** 판정이 불확실하면 **한 단계 위 티어**로 올린다. Tier 0↔1,
  Tier 1↔2 **두 경계 모두**에 적용한다. 하향 추측은 금지한다.
- **검증가능성**을 판정 보조 축으로 명시한다: 결정적 테스트로 전체 커버 가능하고 가역적인 변경은
  Tier 1 판정을 지지하는 요소다(단 하드 신호 해당 시 무효 — 상향 우선 불변).
...
| 외부 발행·프로덕션 데이터 삭제·되돌릴 수 없는 마이그레이션 | 2 | 하드 신호(비가역성) |
...
- **일부 행만 재검증하는 verification skip(verdict 스킵)은 여전히 금지한다.** 이것과 별개로,
  ...
  **실행 결과**는 직전 라운드에서 승계할 수 있다(specs/202607210545) — 출처(`승계: rN@<candidate
  ...
  cross-session 금지 불변. blast-radius 무효화는 base 통합 재평가에 한해 `202607181125`(frozen
  matrix 영향 행 재평가) 정본 소관으로 남으며, 이 규약은 그것을 재정의하지 않는다.
...
- **결정 로그**: 사용자에게 트레이드오프 결정을 받으면(선택지 질문 등) 결정 직후 경량
  ...
  (capture_note — **`tags: ["decision"]` 지정**: 이 태그가 결정 노트의 판별 신호다 —
  brief 구형식 폴백이 이 태그로 수집한다(specs/202607231759). 구 회고 리포트(make retro)도
  같은 태그를 썼으나 great-reduction으로 이관·소멸(specs/032 역사)). 상세는 스펙 문서가
  정본 — 노트는 "왜 그렇게 정했더라?"를 검색으로 소환하기 위한 요약이다(2026-07-05 회고 결정).
