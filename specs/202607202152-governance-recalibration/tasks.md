...
- [ ] **T1.1** deep research 결과 전문(요약·25 claims·caveats·OQ·출처 등급)을 두괄식 human/both
...
- [ ] **T2.1** `~/.localmind/rules/base/interview-protocol.md` 개정(FR-1): 고위험 분기 한정
  검수 대기 + 저위험 decision-by-exception(요약 생략 불가) + 결정 로그 유지 + 개정 이력.
- [ ] **T2.2** `~/.localmind/rules/base/deep-interview-elicitation.md` 개정(FR-2): 권장 기본값
  동반·일괄 제시+예외 교정 모드·기존 종료/한도 유지 + 개정 이력.
- [ ] **T2.3** `~/.localmind/rules/base/governance-recalibration.md` 신설(FR-5): 컷포인트 이동
...
- [ ] **T3.1** 신규 문구 계약 테스트 작성(AC-3·5) — red 확인(TDD: 개정 전 문구 부재로 실패).
- [ ] **T3.2** goal-impl SKILL §4·§4A·§5 재서술(FR-3): 불변식 1급 + 권장 기본 강등 + 재량 문구.
  구역 내 핀 4개 정확 문자열 보존 — `끊김 방어`·`TDD 강제`·`RED 확인 생략 금지`·
  `실패 테스트 먼저(red)`. 구역 밖 불변식은 §5 요약 소절에서 참조 연결만(FR-3 배치 규칙).
- [ ] **T3.3** AGENTS.md 티어 절 정교화(FR-4): 비가역성 하드 신호·검증가능성 보조 축·예시 행.
  **동반 갱신(AC-17 parity)**: 새 신호는 단일 `·`-구분 토큰(괄호 내부는 `/` — 예:
  ...
  함께.
- [ ] **T3.4** 전체 스위트 실행 — 신규 계약 green + 기존 계약 비회귀(AC-4).
