...
describe("FR-5 — judgeEvidenceCarryOver 라운드 간 hermetic evidence 조건부 승계 (AC-5, specs/202607210545)", () => {
  it("교집합 ∅ + hermetic-costly면 승계 가능", () => {
    ...
    assert.equal(result.carryOver, true);
    assert.ok(result.reason.length > 0);
  });

  it("정확 경로 교집합이 있으면 재실행(사유 포함)", () => {
    const result = judgeEvidenceCarryOver({
      declaredDeps: ["src/rules/deploy.ts"],
  ...
  it("디렉토리 접두 교집합(src/rules/ vs src/rules/deploy.ts)이면 재실행(사유 포함)", () => {
    const result = judgeEvidenceCarryOver({
      declaredDeps: ["src/rules/"],
      diffFiles: ["src/rules/deploy.ts"],
      evidenceType: "hermetic-costly",
    });
    assert.equal(result.carryOver, false);
  ...
  it("의존 선언이 없으면(null) 보수적으로 재실행(사유 포함)", () => {
    const result = judgeEvidenceCarryOver({
      declaredDeps: null,
      diffFiles: ["src/other-module.ts"],
      evidenceType: "hermetic-costly",
  ...
  it("cheap 유형은 항상 재실행(사유 포함)", () => {
    ...
    assert.equal(result.carryOver, false);
    assert.match(result.reason, /cheap/);
  });

  it("non-hermetic 유형은 항상 재실행(사유 포함)", () => {
    const result = judgeEvidenceCarryOver({
      declaredDeps: [],
      diffFiles: [],
      evidenceType: "non-hermetic",
    });
    assert.equal(result.carryOver, false);
    assert.match(result.reason, /non-hermetic/);
  });
});
