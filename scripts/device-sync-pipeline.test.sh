...
# great-reduction(2026-07-21): 워크플로 자산 검증(ⓖ verify-targets)·스킬 배포는 sdd-toolkit
# 소관으로 이전 — 수신 워커는 코드·노트·자산 데이터만 다룬다. 코어 검증 게이트(ⓓ)가 실
# 스위트를 실행하도록 src를 복사한다.
rm -rf "$RDIR/src"; cp -R "$ROOT/src" "$RDIR/src"
...
# AC-11(배포 마커)은 great-reduction으로 소멸 — 배포 검증은 sdd-toolkit 소관.
...
# AC-8/12: 성공 경로 — dist 부재 → build → 코어 검증 → restore-assets
