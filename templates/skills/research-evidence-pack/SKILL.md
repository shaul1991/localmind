...
쓰기 전에 출력 경로 자체와 확인된 프로젝트 root부터 출력 경로까지의 모든 `경로 구성요소`를 lstat하고
symlink가 하나라도 있으면 중단한다. 양쪽 `realpath`를 비교해 출력 경로가 프로젝트 root 밖이면 쓰지
않는다. 사용자가 프로젝트 밖 경로를 직접 지정한 경우에도 bundle root 자체가 symlink이면 거부한다.
...
사용자에게 충돌을 보고한다. 경로 검수와 실제 쓰기 사이에 root 또는 구성요소가 symlink로 바뀌면
중단하고 경로를 다시 확인한다.
