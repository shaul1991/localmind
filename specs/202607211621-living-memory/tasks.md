...
- [x] **T0.1** 실패 테스트 먼저: Decision 파싱·직렬화(AC-1 구조), 검증 실패 한국어 에러(AC-3),
...
- [x] **T1.1** capture_note 입력 스키마 확장(choice·why·assumptions 선택 파라미터) →
  decision frontmatter 저장(AC-1). 단일 호출 완결(AC-11).
- [x] **T1.2** 비정형 경로 회귀 테스트 — 파라미터 미지정 시 현행과 동일 구조(AC-2),
  volatility 누락 시 파일 미생성 + 한국어 에러(AC-3 통합 레벨).

## Phase 2 — brief 도구
> depends-on: Phase 1 · files: `src/mcp-server.ts`, `src/mcp-server.test.ts`

- [x] **T2.1** brief 등록 — 힌트로 현행 검색 재사용, 결정 요약(선택·이유·전제 상태) 조립
  (AC-5), 무관/무결과 시 빈 브리핑 한국어 안내(AC-6).

## Phase 3 — 낡음 신호
> depends-on: Phase 2 · files: `src/mcp-server.ts`, `src/mcp-server.test.ts`, `.env.example`
> (BRIEF_STALE_DAYS는 임계 소비 시점인 이 phase에 배치 — advisory ⑦)

- [x] **T3.1** search_notes·brief 응답 끝 한 줄 신호 부가(AC-7 — 본문 무변 검증 포함),
...
- [x] **T4.1** AC-12 문서(결정 캡처·CLAUDE.md 한 줄 brief 연결·재검증 관례) + 전체 스위트
  green.
- [x] **T4.2** 도그푸드: 실제 결정 노트 캡처 → brief 호출 → 신호 관찰(임계 조작으로 stale
  재현) — evidence 기록. **습관화 관찰 항목(advisory ⑧)**: brief가 pull 도구인 한 §6-3의
  "습관 실패" 위험이 잔존 — CLAUDE.md 한 줄 연결이 실세션에서 자동 호출로 이어지는지를
  관찰 항목으로 evidence에 남긴다(판정은 실사용 후).
