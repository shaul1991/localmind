...
> **localmind는 내 AI에게 "나"를 기억시키는 로컬 기억 계층입니다.** 🧠
>
> 누구나 AI에게 일을 맡기는 시대 — 그런데 AI는 매번 내가 누구인지, 무엇을 왜 결정했는지 모른 채
> 새로 시작합니다. localmind를 붙이면 어떤 AI 앱(Claude Code·Claude Desktop·Cursor…)에서든
> 대화의 부산물로 **결정·지식·맥락이 내 `.md` 노트로 저장**되고, 다음 세션의 AI가 **그 기억을
> 가지고 시작**합니다. 노트는 전부 **내 컴퓨터의 파일**이고, git으로 백업·동기화됩니다.
...
- 📝 **기억해 둡니다** — 대화 중 "이거 기록해둬" 하면 AI가 노트로 저장합니다(`capture_note`).
  결정은 **선택·이유·전제** 3층으로 남깁니다 — 왜 그렇게 결정했는지(why)가 가장 값진 기록입니다.
- 🔍 **의미로 찾아줍니다** — "그거 뭐였지?" 하면 내 노트를 의미 기반으로 검색합니다(`search_notes`).
- 🧭 **가지고 시작하게 합니다** — 새 세션의 AI가 작업 주제로 브리핑을 받아, 과거 결정을 이미
  알고 시작합니다(`brief`). "이 프로젝트 뭐 하고 있었지?"를 물을 필요가 없어집니다.
- ⏳ **낡으면 알려줍니다** — 결정이 딛고 선 전제가 오래되면, 그 주제를 다시 만지는 순간 한 줄로
  재검증을 권합니다(비차단 — 흐름을 막지 않습니다).
- 🖥️ **기기를 넘어 다닙니다** — 노트는 git 백업/복구로 어느 기기에서든 같은 기억(`whoami`로 확인).
- 🔒 **전부 로컬** — 데이터는 내 컴퓨터에만. 중앙 서버·공유 계정·클라우드 저장 없음.

## 누구를 위한 건가요?

**AI에게 일을 맡기는 사람 모두 — 개발자가 아니어도 됩니다.** 이 도구를 실제로 조작하는 것은
사람이 아니라 **AI**입니다: 사람은 평범한 대화로 맡기기만 하면("기록해둬 / 찾아줘"), AI가
기록하고·찾아보고·브리핑 받으며 **내 맥락 위에서 일을 더 잘하게** 됩니다. 그렇게 내 지식·결정이
내 컴퓨터에 차곡차곡 쌓이는 **나만의 두뇌**가 생깁니다.
...
- 적용 전 미리보기 `DRY_RUN=1 make mcp-desktop` · **도구 4종**·환경변수 상세 → [MCP로 붙이기](docs/mcp.md).
