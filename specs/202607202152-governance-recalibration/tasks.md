...
- [x] **T4.1** `make rules-deploy` 실행 → `~/.claude/localmind-rules.md`에 개정 문구 grep 관찰
  (AC-7).
- [x] **T4.2** 개정 결정 노트(층별 유지/완화 결정·근거·출처)를 second-brain에 capture(AC-8,
  tags: decision).
...
- [x] **T5.1** preflight 실행 후 **렌즈 병렬 fan-out self-review**(5렌즈 격리 critic 동시 —
  첫 실전, merged report 하나 = round 1개, evidence `lenses`·`duration-minutes` 기록).
- [x] **T5.2** AC-1·2·6(수동 검사 대상)을 리뷰어가 실파일 대조로 판정했는지 확인하고, 세 문서
