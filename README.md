...
### 공용 Deep Research 워크플로

`deep-research`는 같은 조사 행동을 공유하는 **공용 logical command ID**입니다. Agent Skills를
지원하는 런타임에 배포한 뒤 주제만 붙여 호출합니다. 조사 중에는 `R-001`·`S-001`·`E-001`·`C-001`
형식의 안정 ID로 출처·근거·주장을 연결하고, 고정 출처 수 대신 권위·독립성·상충 여부로 충분성을
판정하며, 긴 조사는 checkpoint로 이어갈 수 있습니다.
...
Codex의 bare `/deep-research`는 공식 호출 문법이 아니므로 지원하거나 권장하지 않습니다. deprecated
Custom Prompts 경로도 사용하지 않습니다. 이 워크플로는 벤더의 first-party Deep Research와 별개이며,
전용 backend나 전용 모델을 복제하는 기능이 아닙니다. 자세한 사용 범위와 조사 품질·안전 경계는
[Deep Research 워크플로 안내](docs/workflows.md#deep-research--근거-기반-심층-조사)를 보세요.

이미 완료한 조사를 기계 검증 가능한 파일로 남길 때만 별도 explicit·docs-only 워크플로인
`research-evidence-pack`을 호출합니다. Claude Code는 `/research-evidence-pack <confirmed-path>`,
Codex는 `$research-evidence-pack <confirmed-path>`를 사용합니다. 실행 validator가 필요한 이 워크플로는
공용 Agent Skill target에는 배포되지만 Gemini용 generated wrapper는 만들지 않고
`skipped-dependency`로 정직하게 보고합니다.
...
- 기본은 **기기마다 로컬(stdio)** 로 붙는 것입니다(위 표의 `make mcp-install` 등). 노트를 홈서버 한
  대에 모아두고 여러 기기에서 원격으로 붙고 싶다면 **원격(http) 접속**도 선택할 수 있어요 →
  [홈서버 가이드](docs/home-server.md).

**웹으로 둘러보기(선택)** — `make ui` (브라우저 자동으로 열려요)에서 스택 상태·**연결 상태**(무엇이
연결됐고 바꿀 명령은 뭔지)·**내 노트를 카드로** 봅니다(읽기 전용). 터미널을 계속 쓰려면
`make ui-bg`(백그라운드로 켜기 → 끄기 `make ui-stop`). Node.js가 필요해요(macOS: `brew install node`).
...
- 🏠 [홈서버 · 휴대폰](docs/home-server.md) — 홈서버에 중앙집중 + Tailscale·SSH로 휴대폰에서 쓰기
- 🎭 [페르소나 에이전트](docs/agents.md) — 역할·모델을 정의해 배포·싱크
- 🧩 [AI 워크플로 자산](docs/agents.md#5-공급자-중립-ai-워크플로-자산-specs044) — `make skills-deploy` 한 번으로 `deep-research`를 포함한 현재 packaged catalog를 Claude Code·Codex·Gemini CLI에 배포(공급자 중립)
