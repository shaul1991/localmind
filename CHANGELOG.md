...
- **빌드·자격증명 격리**: CI gate 직후 deploy token을 제거하고, 전용 builder의 기존·잔존 프로세스를 fail-closed로 검사·종료한 뒤에만 root-owned release로 승격한다.
...
- prune-guard 자식 reindex의 embed fetch가 병렬 스위트 경합에서 120s 상한 즉사하던 것을
  15s×3회 재시도로 흡수(테스트 env만 — 제품 코드 무변경). 4회 관찰 플레이크의 첫 로그 포착·수정.
