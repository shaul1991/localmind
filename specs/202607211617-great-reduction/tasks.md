...
- [x] **T2.0** (critic B1 보충 — **절제 전 필수 선행**) Codex/Gemini/페르소나 교차 parity:
...
- [x] **T2.1** Extract·Remove 파일 삭제(inventory 목록 전수 — **게이트웨이 스택 12파일 명시
  ...
  *(집행 중 개정: config.ts·types.ts는 절제 후 소비자 0 실측으로 전체 Remove — spec amendment
  r0-4가 정본, 이 지침보다 우선. r1 렌즈① advisory 반영 부기.)*
- [x] **T2.2** 부분 절단(coupling 절단선): mcp-server 도구 12개 절제(AC-1 단언 테스트 선행 —
...
- [x] **T2.3** AC-2 grep 검증 스크립트 + AC-5 inventory 대조 스크립트 작성·실행 — 결정적,
  evidence 보존.

## Phase 3 — 문서 정합
> depends-on: Phase 2 · files: `README.md`, `docs/`

- [x] **T3.1** Keep docs 12 개정 — 제거된 도구·서비스·타깃 언급 소거(FR-6, 역사 문서 제외).
...
- [x] **T4.1** localmind 전 스위트 green(AC-4) — 잔존 테스트가 Extract 모듈 참조 0.
- [x] **T4.2** 재빌드 → stdio 실호출 도그푸드(AC-6): whoami·capture_note(태그 호출자 공급)·
...
- [x] **T5.1** preflight → 격리 렌즈 critic(§7A 예산 2라운드) → clean → 문서 검증 표기 →
