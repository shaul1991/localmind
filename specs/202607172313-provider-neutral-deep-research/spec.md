...
- [x] **FR-1 — canonical package와 catalog 등록:** `deep-research`는 하나의 canonical Agent Skill과
...
- [x] **FR-2 — provider/model/tool neutrality:** canonical skill과 reference는 특정 provider,
...
- [x] **FR-3 — runtime-native invocation parity:** 현재 LocalMind 지원 대상에서 같은 logical ID를
...
- [x] **FR-4 — explicit activation·input gate:** 명시 호출만 workflow를 시작한다. 주제 인자가 없거나
...
- [x] **FR-5 — research brief와 확인 gate:** workflow는 조사 전에 질문·목적·대상 독자·기준 시점·
...
- [x] **FR-6 — 기존 문맥 회수와 source strategy:** 적용 가능한 project instruction, 제공 파일,
...
- [x] **FR-7 — 조사 질문 분해·fan-out·barrier:** brief를 독립 research questions로 분해한다.
...
- [x] **FR-8 — live evidence와 source authority:** 시간 민감 claim은 최신 공식·표준·원자료를 현재
...
- [x] **FR-9 — 상충 근거·사실/추론/미검증 구분:** 상충 source를 조용히 하나로 합치지 않는다.
...
- [x] **FR-10 — conclusion-first report:** 기본 출력은 채팅 보고다. 최소 구조는 TL;DR, research
...
- [x] **FR-11 — critic gate:** 모든 research lane과 evidence ledger가 모인 뒤 isolated critic을
...
- [x] **FR-12 — report-only safety boundary:** 조사 중 source·repository·외부 서비스의 상태를
...
- [x] **FR-13 — lifecycle·문서·drift 보호:** 기존 catalog-driven seed/deploy/verify/prune
...
- [x] **FR-14 — execution tier·role routing:** workflow는 source scout를 `economy`, research
...
검증 근거: FR-1~3·13은 catalog/adapter/lifecycle 테스트와 설치본 hash, FR-4~12는 canonical
계약 테스트와 [행동 감사 증거](evidence.md), FR-14는 neutrality/tier 정적 테스트와 두 runtime의
truthful fallback 기록으로 확인했다. FR별 primary task는 [tasks.md](tasks.md)에 연결돼 있다.

## Acceptance Criteria

- [x] **AC-1 — package contract (FR-1·2):** Given packaged skills를 로드할 때, When catalog와
...
- [x] **AC-2 — runtime invocation (FR-3):** Given 세 현재 지원 target의 deploy 결과에서,
...
- [x] **AC-3 — explicit metadata (FR-1·3·4):** Given `explicit` policy를 배포할 때, When target
...
- [x] **AC-4 — no-topic and non-activation edges (FR-4):** Given 주제 없는 명시 호출 또는
...
- [x] **AC-5 — brief confirmation (FR-5):** Given 유효한 명시 호출에서, When broad research 직전,
...
- [x] **AC-6 — live evidence (FR-6·8):** Given 가격·버전·지원정책처럼 시간 민감한 claim을 조사할
      때, When 보고를 작성하면, Then 핵심 claim마다 직접 확인한 URL과 확인일/source authority가
      연결되고 T1/T2가 우선되며, 미확인 source나 T4 단독 핵심 결론은 0건이다.

- [x] **AC-7 — fan-out and barrier (FR-7):** Given 서로 독립인 유의미한 research question이 2개
...
- [x] **AC-8 — conflicts and epistemic labels (FR-9):** Given 권위 있는 source끼리 내용이 충돌할 때,
      When 최종 보고를 보면, Then 양쪽 source·날짜·적용 범위가 노출되고 채택/보류 근거가 있으며,
      사실·추론·권고·미검증 항목이 구분된다.

- [x] **AC-9 — degraded mode (FR-6·9):** Given live source lookup capability가 없을 때, When workflow를
...
- [x] **AC-10 — report shape (FR-10):** Given 완료된 조사에서, When 기본 보고를 검토하면, Then
...
- [x] **AC-11 — critic truthfulness (FR-11):** Given 모든 research lane 완료 후, When 최종 보고 직전,
      Then critic checklist가 항상 실행되고 명백 결함은 수정·재검된다. 격리 reviewer를 실제 사용한
      경우에만 independent로, 아니면 `not independent`로 표기된다.

- [x] **AC-12 — report-only boundary (FR-12):** Given 저장·변경을 별도로 요청하지 않은 조사에서,
...
- [x] **AC-13 — lifecycle safety (FR-13):** Given 빈 임시 target roots와 동명 unmanaged fixture에서,
...
- [x] **AC-14 — discoverability (FR-3·13):** Given README와 workflow 문서를 읽을 때, When 비개발자가
...
- [x] **AC-15 — representative dogfood (FR-5~11):** Given 하나의 시간 민감 비교 주제로 구현물을
...
- [x] **AC-16 — tier routing and fallback (FR-14):** Given 단계별 역할 배치를 검사할 때, When
...
검증 근거: AC-1~3·13~14는 자동 계약·배포·문서 테스트, AC-4~12·15~16은
[activation/conflict/security/Claude/Codex 감사 자료](evidence.md)로 1:1 확인했다. 최종 회귀는
전체 922/922이며 상세 AC→task→evidence 매핑은 [tasks.md](tasks.md)에 있다.
...
- **OQ-2 (구현 착수 시 spike):** 지원 대상 Gemini CLI 설치본에서 native skill slash와 generated
  ...
  유지한 채 위험을 명시한다.
  **상태: 미해결·비차단.** 현재 설치 환경에 실행 가능한 Gemini CLI가 없어 실제 우선순위는 검증하지
  않았고, generated wrapper의 정적 계약만 통과시켰다. 후속 runtime 편입 slice에서 live spike한다.
