...
> (capture·검색·백업·동기화 + MCP 도구 3종)에 집중합니다 — 배경: [product-vision](product-vision.md).
...
| `EMBEDDINGS_URL` | `http://localhost:4000/v1` | 노트 임베딩 엔드포인트(OpenAI 호환) — **권장: Ollama 직결 `http://localhost:11434/v1`** |
...
- 더 가벼운 모델은 **반드시 다국어**(multilingual-e5, snowflake-arctic-embed2) — 영어 전용은
...
```bash
make smoke        # MCP 도구 표면(3종) + brain(capture·검색) 스모크
