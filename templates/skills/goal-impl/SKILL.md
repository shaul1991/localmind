...
이 워크플로는 **Tier 2(실질적 변경) 구현 흐름의 정본**이다. 진입 시 AGENTS.md "변경 등급 티어"
절을 정본으로 변경 등급(티어)을 인지하고, 그에 맞춰 검수 강도를 조정한다.

- **Tier 1(작음)** — 이 워크플로의 전체 의식을 그대로 쓰지 않고 낮춘다: **in-session 적대
  자기검증 1라운드**(격리 위임 없이 현재 세션이 결함을 찾으러 가는 자세로 diff 스코프를
  재검토)로 검수하며, §7A의 자동 2라운드 상한·격리 리뷰어 요구는 적용하지 않는다.
...
**round-to-round 무효화-스킵(적극형 — 특정 조건에서 일부 행만 재검증하는 방식)은 이 워크플로에
도입하지 않는다.** blast-radius 무효화 판정은 §3A(base freshness gate)의 "base 통합으로
candidate가 변경되면 frozen matrix의 영향 행을 재평가" 규약에 **한해서만** 적용되며, 그 규약을
여기서 재정의하지 않는다.
