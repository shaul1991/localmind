...
1. **인터뷰 게이트(구현 착수 전)**: spec Open questions 6건을 사용자에게 확인하고
   "확정 사항"으로 이관한다. 문서 없이/추측으로 구현하지 않는다(AGENTS.md 규약 6).
   결정 직후 경량 결정 노트를 second-brain에 적재한다(AGENTS.md 결정 로그 규약).
