...
- [ ] **T1.1** AC-5 실패 테스트: `judgeEvidenceCarryOver` 5케이스({교집합∅+hermetic-costly}→승계,
  {교집합 존재}·{선언 부재}·{cheap}·{non-hermetic}→재실행+사유) 작성(red).
- [ ] **T1.2** 판정 순수 함수 구현 → green. 입력·출력·규칙은 spec FR-5와 1:1.
- [ ] **T1.3** AC-6 실패 테스트 + `carried-from` 집계(선택 필드·미준수 불참여)·§8 렌더(승계
...
- [ ] **T2.1** 계약 테스트 선행 개정(red 관찰): AC-12 구 핀 2곳(sdd-self-review·goal-impl) →
  새 조건부 문구 핀 교체 + **신규 핀 추가**(루트 AGENTS.md·`templates/sdd/AGENTS.md` 스캐폴드
  — 기존 핀 없음, 3조건·보수 기본·출처 표기·저비용 재실행). AC-11·13 기존 핀은 손대지 않음.
- [ ] **T2.2** 루트 AGENTS.md + `templates/sdd/AGENTS.md` critic 캐싱 절 동반 개정(FR-1) —
  verdict 관련 불릿 불변, 대상 불릿만 교체(202607181125 carve-out 문장 보존).
- [ ] **T2.3** sdd-self-review 2A 승계 절차 추가(FR-2) + goal-impl §7A 정합 개정(FR-3 — §3A
  carve-out 보존) + evidence 템플릿 선택 필드 주석 3필드 갱신(FR-5).
- [ ] **T2.4** Phase 2 산출 단독 검증 — workflow-policy 계약 테스트 파일만 실행해 신규·교체
...
- [ ] **T3.0** 전체 스위트 실행 — 양 phase 산출 통합 green + 기존 계약 비회귀(AC-4의 실행
  지점 — Phase 1·2 완료 후라 DAG 선언과 일치).
- [ ] **T3.1** dogfood: preflight 실행·retro 실행으로 승계 건수 절 출현 관찰. 판정 함수를 실제
