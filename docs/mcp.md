...
> 호스트 AI가 검색 결과로 직접 수행합니다. living-memory 2026-07: 결정 캡처·brief·낡음 신호 추가.)

**세션과 연결하기(권장)** — 쓰는 AI 도구의 지침 파일(예: `CLAUDE.md`·`AGENTS.md`)에 한 줄:

```
새 세션 시작 시, 작업 주제로 localmind의 brief 도구를 호출해 과거 결정을 확인한다.
```

**전제 재검증 관례** — 낡음 신호(⏳)를 받으면 호스트 AI가 최신 사실을 확인한 뒤, 신호에 적힌
노트 파일의 해당 전제 `last_verified`를 오늘 날짜로 직접 수정합니다(노트는 평문 파일 — 전용
도구 없음). 갱신은 다음 조회부터 바로 반영됩니다. 신호 임계는 `BRIEF_STALE_DAYS`(기본 30일).
